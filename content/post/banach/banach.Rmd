---
title: "Verificando o Teorema do Ponto Fixo de Banach no R"
...
Essa proposição deve ter uma infinidade de provas diferentes. Eu particularente gosto da prova que me veio à mente quando um professor de análise apresenteou esse problema em sala. 

Um mapa $f$ só tem ponto fixo se cruza a função identidade, a famosa $g(x)=x$, cuja derivada sempre é $1$ e tem infinitos pontos fixos. Então queremos mostrar que se uma função real _nunca_ tem derivada igual a $1$, ela só pode cruzar a função identidade no máximo uma vez. Segue aqui um esboço:

Suponha por absurdo que $f$ tenha dois pontos fixos, logo duas interseções com a identidade. Digamos que são $x_1$ e $x_2$. Agora usamos um "lema", o Teorema do Valor Médio. Ele nos garante que dada uma função contínua definida num intervalo fechado $[a,b]$, existe algum ponto $c$ tal que $f'(c) = \frac{f(b)-f(a)}{b-a}$. Em bom português: uma função contínua que varia a uma certa taxa média em um intervalo vai ter derivada igual a essa taxa média em algum ponto desse intervalo.

Opa opa, então você estamos dizendo que $f$ passou por um intervalo fechado $[x_1, x_2]$ em que teve taxa de variação média de $1$ - já que $f(x_1)$ e $f(x_2)$ estão ambos sobre a reta da identidade. Pelo Teorema do Valor Médio, a derivada de $f$ necessariamente assumiu o valor $1$ em algum ponto desse intervalo, o que contradiz nossa hipótese inicial. Temos então um absurdo e aqui termina a "prova". 
...
Vamos quebrar um pouco o enunciado. Se existe o $A$ descrito, então $f$ é dita uma _contração_. A distância da imagem de quaisquer do domínio é sempre menor ou igual a distância entre os próprios pontos. $f$ é um mapa que comprime distâncias. 

Se $x_1$ é um número real arbitrário e a sequência definida daquela maneira tem um limite, então estamos dizendo que se aplicarmos uma contração em um ponto repetidas vezes, essa sequência converge para um ponto - e mais interessante ainda - _somente_ para esse ponto, independente do ponto inicial escolhido. 

Essa proposição diz que se $f$ é uma contração, ela não só tem ponto fixo, como tem _um único_ ponto fixo e que podemos encontra-lo aplicando a função repetidas vezes com um ponto inicial qualquer.

Segue um esboço da prova:

$d(x,y)$ é a distância euclidiana entre dois pontos. Uma métrica - afinal estamos falando de funções em espaços métricos. 
...
$$d(x,y) \leq \frac{(x, f(x))  + d(y, f(y))}{1 - A}$$
...
Esse teorema, às vezes chamado de _Contraction Mapping Theorem_ é um resultado muito importante para economia, especialmente porque garante que várias técnicas de programação dinâmica tenham solução. Agora vamos sair da teoria, ver brevemente algumas aplicações de pontos fixos e depois e por as mãos na massa, observar ele acontecendo na prática.

# Por que pontos fixos são interessantes?

Idealmente essa seria a primeira seção do post, mas eu queria ter certeza que o leitor sabia o _o que é_ um ponto fixo. Agora sabe. Os familiarizados com álgebra linear vão se lembrar do conceito de autovetor. Pois, um ponto fixo é essencialmente um autovetor cujo autovalor é $1$. São "locais estáveis", por assim dizer, de funções. 

Kim Border, em seu livro _Fixed Point Theorems with Applications to Economics and Game Theory_ diz:
...
of proving the existence of a solution to a set of (nonlinear) equations.
...
obj1 = contracao(c = 10, A = .05, n = n) 
obj2 = contracao(c = 10, A = .2, n = n)
obj3 = contracao(c = 10, A = .6, n = n)
obj4 = contracao(c = 10, A = .8, n = n)
x = seq(0, n-1, by = 1)
...
Podemos manter a taxa de convergência constante e alterar as posições iniciais também.
...
obj5 = contracao(c = 10, A = A, n = n) 
obj6 = contracao(c = 100, A = A, n = n)
obj7 = contracao(c = 1000, A = A, n = n)
obj8 = contracao(c = 10000, A = A, n = n)
x = seq(0, n-1, by = 1)

dados = data.frame(Quinto = obj5, Sexto = obj6,
                   Setimo = obj7, Oitavo = obj8,
                   x = x)

ggplot(dados, aes(x = x))+
 geom_line(aes(y = obj5), size = 2)+
  geom_line(aes(y = obj6), color = "blue", size = 2)+
  geom_line(aes(y = obj7), color = "green", size = 2)+
  geom_line(aes(y = obj8), color = "yellow", size = 2)
