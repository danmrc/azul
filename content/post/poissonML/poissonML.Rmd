...
* Seja $X$ um conjunto de variáveis aleatórias $X_i \sim \text{Pois}(\lambda_i)$, então$\sum_{i=1}^n X_i \sim \text{Pois}(\sum_{i=1}^n \lambda_i)$. A soma de várias Poisson, cada uma com $\lambda_i$ gera uma Poisson cujo parâmetro é a soma dos $\lambda_i$ individuais.
...
Esse trabalho todo para chegar na média. É isso, o melhor chute para a taxa de ocorrência verdadeira é a média das taxas de ocorrência observadas. Faz sentido... eu acho.

Tá agora vamos ver um pouco da magia acontecendo, coisa e tals. Primeiro simular uma amostra com um $\lambda$ positivo aleatório que precisaremos descobrir.
