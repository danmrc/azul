---
title: "A Abordagem de Ponto Fixo para o Teorema de Perron-Frobenius Parte I: Dois
...
* *Definição:* O *$n$-simplex padrão* (denotado por $\Delta_n$) é o conjunto de combinações convexas de $n+1$ vértices $\{v_1,v_2,...,v_{n+1}\}$. Explicitamente, $\Delta_n = \{ \sum_{i=1}^{n+1} \theta_i v_i \,\,;\,\, \sum v_i = 1 \,\, \& \,\, \theta_i > 0 \,\, \& \sum \theta_i = 1\}$.
...
* *Definição:* Uma subdivisão baricêntrica simplicial de um $n$-simplexo é uma partição de $\Delta_n$ em simplexos menores (que chamaremos de *células*) de forma que qualquer dos simplexos menores seja disjunto ou compartilhe apenas uma única face. 
...
```{r echo=FALSE, out.width='100%', fig.cap= "Uma coloração de Sperner para o $2$-simplexo"}
...
A prova em uma dimensão divertida e perspicaz, a prova do caso geral já não me evoca grandes reações então deixarei apenas um esboço aqui. É um resultado conhecido e o leitor não terá dificuldades em encontrar uma demosntração completa e rigorosa.
...
Observe que isso converge a zero, à medida que $n,m \to \infty$. Afinal:
