...
Existem várias maneiras de abordar o problema de programação dinâmica, e o assunto pode ficar altamente matemático rapidamente. Uma maneira "padrão" de resolver é escrever o problema acima como um lagrangeano padrão e tirar a derivada em relação a variável de controle no tempo t, $k_t+1$. Para este caso, podemos reescrever a restrição para isolar o $c_t$ e substituir no problema. Com isso, temos:
...
Se você ainda não entendeu para onde isso está indo, eu pretendo escrever o problema de maximização de uma data $t$ qualquer como um problema que depende de $V_{t+1}(k_{t+1},c_{t+1})$. Para facilitar a nossa vida, vamos chamar $V_t(k_t,c_t)$ de função valor (por isso V): ela dá o valor do problema de otimização amanhã. Logo, o problema na data $t$ é:
