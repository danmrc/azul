...
Por enquanto só falamos de VARs que tem uma defasagem do vetor $\mathbf{x_t}$. Podemos ter VARs de qualquer ordem: um VAR(2) depende de $\mathbf{x_{t-1}}, \mathbf{x_{t-2}}$ etc. Selecionar a ordem do VAR não é trivial e normalmente é feito usando algum critério de informação. Mas podemos obter toda a intuição com um VAR(1). Veja que podemos estimar a matriz $C$ estimando cada uma das equações por MQO e obter:
