...
Veja que do erro quadrático médio pode ser decomposto em duas parte, uma sendo a variância do estimador e outra que é o viés:
...
1. O estimador James-Stein realmente garante que o estimador de OLS não é admissível

2. $p-2$ é o valor ótimo para $c$. 

Eu vou implementar três funções que vão fazer as simulações necessárias. Todos os parâmetros do modelo vão ter o mesmo valor em cada simulação.

A primeira delas pega um valor pro parâmetro verdadeiro, um conjunto de valores para o parâmetro $c$ (que eu chamei, talvez confusamente, de `regs`, para regularização), o tamanho da amostra $n$ e a quantidade de variáveis (além do intercepto) $k$:
...
Novamente, $\hat{\beta}^T X^T X \hat{\beta}$ é um escalar. O estimador não faz encolhimento do intercepto (eu não vou dar nenhuma explicação para isso, mas o `glmnet` também não encolhe o intercepto). 
...
Veja que o ponto 2 não é nenhum problema (computacional, pelo menos). O ponto 1 é um pouco mais preocupante, especialmente se seu interesse é interpretar o coeficiente como um efeito de tratamento. Mas isso ilustra que, para previsão, mínimos quadrados não necessariamente é ótimo. 
...
Este é um destes posts que começam com um tema meio abstrato (teoria da decisão) e desaguam em um resultado completamente maluco (MQO nunca deveria ser usado, dependendo do seu objetivo). A gente nem precisou fazer conta nenhuma, o computador fez tudo. Eu não fiz essas contas para o LASSO porque senão o post ia se esticar ainda mais.

--- 

# Bibliografia

O capítulo 7 do [Computer Age Statistical Inference](https://web.stanford.edu/~hastie/CASI/) discute exatamente o estimador James-Stein, dando uma motivação Bayesiana. A fórmula que eu usei no post pro James- Stein veio de lá. A discussão está mais próxima do Econometric Foundations, apesar de eu ter sérias dúvidas sobre a fórmula que eles colocam para o estimador James-Stein. 