...
(A prova disso é o pesadelo de muitos alunos de pós graduação)

Uma correspondência é uma generalização de uma função: pensar f como uma função é totalmente inóquo para o presente exemplo (o caso do teorema com função, ao invés de uma correspondência, se chama Teorema do Ponto Fixo de Brouwer). No caso de ser uma função, exigimos continuidade. Ser _compacto_, nos reais, significa ser fechado e limitado. Logo $[0,1]$ é um compacto, mas $(0,1)$ não. E para $\mathbb{R}^2,\mathbb{R}^3, \dots$ a definição permanece, logo $[0,1] \times [0,1]$ é um compacto em $\mathbb{R}^2$. Um pouco frustrante, entretanto, é que o ponto fixo de Banach nos fornece um algoritmo para encontrar o ponto fixo: faça uma sequência de $x$ tal que $x_i = f(x_{i-1})$. O limite dessa sequência vai ser o ponto fixo da função. O ponto fixo de Kakutani não nós dá nada desse tipo. Mas no nosso caso, _iterar a sequência vai nos levar ao ponto fixo_.
...
````
phi_hat (generic function with 2 methods)
````
...
{{< figure src="../eq_geral_e_ponto_fixo/ponto_fixo_e_eq_geral_4_1.svg"  >}}

Claramente o problema converge para a solução verdadeira. Isso é interessante porque mostra que, para esse caso, um processo de aprendizado que envolve reagir ao último preço leva o mercado a um eventual equilíbrio - uma ideia vinda de Walras que recebe o nome de tâtonnement. Veja que mesmo preços absurdamente distantes funcionam:
...
{{< figure src="../eq_geral_e_ponto_fixo/ponto_fixo_e_eq_geral_5_1.svg"  >}}
...
{{< figure src="../eq_geral_e_ponto_fixo/ponto_fixo_e_eq_geral_6_1.svg"  >}}
...
{{< figure src="../eq_geral_e_ponto_fixo/ponto_fixo_e_eq_geral_7_1.svg"  >}}
...
{{< figure src="../eq_geral_e_ponto_fixo/ponto_fixo_e_eq_geral_8_1.svg"  >}}
...
{{< figure src="../eq_geral_e_ponto_fixo/ponto_fixo_e_eq_geral_9_1.svg"  >}}
