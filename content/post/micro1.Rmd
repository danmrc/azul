...
cobb.douglas.2 <- function(x, 
  ...
  cobb.douglas = list(y=y,alpha=alpha,beta=beta)
  }
...
x1 = sort(rep(x1, length(x1)), decreasing = FALSE)

x2 = seq(from=1, to = 100, by = 1)
x2 = rep(x2,length(x2))

consumo = data.frame(x1,x2)

util = cobb.douglas.2(consumo, alpha = .1)
u = util$y
...
scatter3D(x=x1, y=x2, z= u, 
       xlab = "x_1", ylab = "x_2", zlab = "Utilidade")
...
Observe que fazendo curvas de nível você consegue aquele clássico formato de curvas de indiferença em funções de utilidade Cobb-Douglas. 
