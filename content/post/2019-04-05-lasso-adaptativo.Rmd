...
3. Crie um vetor que tem a quantidade de coeficientes não zeros para cada modelo estimado. Vamos nos referir a cada entrada desse modelo como $s$.
4. Calcule a soma do quadrado dos erros para cada modelo $SSR = \sum_{i=1}^{n} u_i^2$. 
5. Calcule $IC = n*ln(SSR) + c*s$ para cada modelo, onde c é o critério de informação escolhido
