...
Vou fazer uma simulação de um caso bem simples mostrando isso, simular um amostra com média 0.1 de uma normal e testar para a hipótese nula de média 0. Vou colocar a variância um pouquinho alta para reduzir o poder:
...
Nós queremos $P(\hat{\beta}|\hat{\beta}/SE > c_{\alpha})$. Esse termo que está condicionando não tem distribuição normal padrão porque faltou subtrair a média, então acertando isso nós teremos $(\hat{\beta} - \beta)/SE > c_{\alpha} - \beta/SE$ e substituindo os termos na fórmula que tá na Wikipedia nós teremos:
