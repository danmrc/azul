...
Uma propriedade conveniente da representação no domínio da frequência é que é fácil saber o que acontece com uma série se passamos por múltiplos filtros: basta multiplicar as representações no domínio da frequência. Então o efeito de passar a séria $x_t$ pelos filtro $\alpha(B)$ e $\beta(B)$ é simplesmente $\alpha(e^{-iw})\beta(e^{-iw})$. Isso será de grande valia no que se segue. 
...
Implementando no R usando a nossa regra de bolso:
...
plot(x,Arg(ma_freq_domain(x))/x,type="l",ylab = " ", xlab = "Frequência", main = "Fase do Filtro MA")
grid(col = "darkgrey")
```
...
Veja que não há nenhum número complexo ai. Isso se deve ao fato de que o filtro HP é um filtro _simétrico_. Um filtro é dito _simétrico_ se, na representação no domínio temporal $a_j = a_{-j}$.(Lembrando que o filtro é definido como $\displaystyle \sum_{j=-\infty}^{\infty} a_j B^{j}$)

Nesse caso, o filtro tem a propriedade de _neutralidade de fase i.e._, $\frac{\theta}{\omega} = 0$. Noutras palavras, o ganho resume tudo que o filtro faz. Vamos plotar o ganho do filtro HP:
