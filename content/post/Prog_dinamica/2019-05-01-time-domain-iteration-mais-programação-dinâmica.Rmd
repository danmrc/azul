...
$$-u(c_t) + (1+r)\beta u(c_{t+1}) +\lambda_t = 0$$
...
1. Compute qual é o máximo de dívida disponível para esse período usando a equação (3)
...
3. Compute o valor de $-u(c_{max}) + (1+r)\beta E(u(c(b_{t+1})))$. Se o valor for maior que 0, pule para o item 5
4. Encontre o valor que faz com que $-u(c_t) + (1+r)\beta u(c(b_{t+1})) = 0$. 
5. Salve o valor de c_t
6. Proceda até convergência

Veja que esse algoritmo é ligeiramente diferente do algoritmo do Bianchi em alguns pontos - e porque o problema que eu estou resolvendo aqui no blog é mais simples que o problema dele. 