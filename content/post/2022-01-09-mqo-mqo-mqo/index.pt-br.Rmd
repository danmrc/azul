...
$$
\frac{1}{n}\left<u^TX,(\hat{\beta} - \beta_0)\right> \leq \frac{1}{n}|\left<u^TX,(\hat{\beta} - \beta_0)\right>| \frac{1}{n} \leq \|u^TX\|_{\infty} \|\hat{\beta} - \beta_0\|_1
$$
...
$$
P\left(\max_{i=1,\ldots,p} \frac{|u^Tx_i|}{n} > t\right) \leq p\exp \left(-\frac{t^2}{2\sigma^2/n^2}\right)
$$

Eu assumi que a variância da coluna é 1 (ou seja, nós normalizamos a coluna, como a maioria dos pacotes de machine learning faz). Para $t = \frac{\sigma}{n}\sqrt{2(\log(p) + \delta)}$, nós temos:

$$
P\left(\max_{i=1,\ldots,p} \frac{|u^Tx_i|}{n} > t\right) \leq p\exp \left(-\frac{2\sigma^2/n^2(\log(p) + \delta)}{2\sigma^2/n^2}\right) = p\exp \left(-\log(p) - \delta\right) = \exp(-\delta)
$$

Com alta probabilidade, $\frac{\sigma}{n}\sqrt{2(\log(p) + \delta)}$ é maior que o máximo. Vamos substituir esse valor na nossa cota: 

$$
\left<u^TX,(\hat{\beta} - \beta_0)\right> \leq |\left<u^TX,(\hat{\beta} - \beta_0)\right>| \leq \|u^TX\|_{\infty} \|\hat{\beta} - \beta_0\|_1 \\
\left<u^TX,(\hat{\beta} - \beta_0)\right> \leq \frac{\sigma}{n}\sqrt{2(\log(p) + \delta)}\|\hat{\beta} - \beta_0\|_1
$$

Lembra que eu argumentei que $\|x\|_1 \leq \sqrt{n}\|x\|_2$ (isso foi o ponto 5)? Vamos usar isso agora com $\|\hat{\beta} - \beta_0\|_1$:

$$
\left<u^TX,(\hat{\beta} - \beta_0)\right> \leq \frac{\sigma}{n}\sqrt{2(\log(p) + \delta)}\|\hat{\beta} - \beta_0\|_1 \leq \frac{\sigma}{n}\sqrt{2(\log(p) + \delta)}\sqrt{n}\|\hat{\beta} - \beta_0\|_2
$$

Vamos jogar isso de volta na equação 1:

$$
\frac{1}{n} \|X(\hat{\beta} - \beta_0)\|_2^2 \leq 2 \sigma\sqrt{\frac{2(\log(p) + \delta)}{n}}\|\hat{\beta} - \beta_0\|_2
$$

Multiplique e divida o lado direito por $\|\hat{\beta} - \beta_0\|_2$:

$$
\frac{1}{n} \|X(\hat{\beta} - \beta_0)\|_2^2 \leq 2 \sigma\sqrt{\frac{2(\log(p) + \delta)}{n}}\frac{\|\hat{\beta} - \beta_0\|_2^2}{\|\hat{\beta} - \beta_0\|_2}
$$

Reorganizando:

$$
\frac{1}{n} \frac{\|X(\hat{\beta} - \beta_0)\|_2^2}{\|\hat{\beta} - \beta_0\|_2^2} \leq 2 \sigma\sqrt{\frac{2(\log(p) + \delta)}{n}}\frac{1}{\|\hat{\beta} - \beta_0\|_2}
$$

Uma boa hora para usar o nosso resultado 3, sobre o autovalor da matriz:

$$
\lambda_{\min} \leq \frac{1}{n} \frac{\|X(\hat{\beta} - \beta_0)\|_2^2}{\|\hat{\beta} - \beta_0\|_2^2} \leq 2\sigma\sqrt{\frac{2(\log(p) + \delta)}{n}}\frac{1}{\|\hat{\beta} - \beta_0\|_2}
$$

Reorganizando a expressão acima:

$$
\|\hat{\beta} - \beta_0\|_2 \leq \frac{2\sigma}{\lambda_{\min}}\sqrt{\frac{2(\log(p) + \delta)}{n}}
$$
