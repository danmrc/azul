...
Dia desses li coisas tristes. A narrativa era de que alguns setores são por alguma propriedade vinda dos céus (alguns dirão _ah mas e a complexidade..._ e eu direi que são eles os que invejam os físicos) mais "importantes" que outros e que, de fato, o processo de desenvolvimento econômico ?  de substituir participação de setores menos complexos por outros mais complexos. A magia, o pulo do gato, o estopim de um ciclo virtuoso de crescimento estaria em produzir menos soja e mais massa proteica, menos ferro e mais carros, menos bananas e mais microchips... Qualquer semelhan?a com as vi?vas do regime militar ~não~ ? coincidência. 

Este blogueiro discorda e gostaria de mostrar (i) como R (e mais geralmente, programação) facilitam nossa vida e (ii) que composição setorial não ? lá grandes coisas. Decidi usar a 10-Sector Database com dados de emprego e produto a nível de setor em vários países e a Penn World Tables, com algumas explicativas macroeconômicas. A 10-Sector tem um formato bem esquisito. Você recebe um dataset em que cada linha é uma combinação de ano-país-variável, em que as variáveis podem ser algumas medidas de valor adicionado ou população empregada no setor. Ela entrega 11 colunas, 10 representando setores e uma o agregado.

O leitor rapidamente percebe que s?o dados com uma estrutura um tanto quanto curiosa, nem um pouco _tidy_ em que cada coluna é uma variável e cada linha uma observação. Vamos tomar o cuidado de antes de qualquer coisa, ter dados neste formato. Antes de mais nada, vamos inspecionar a documentação, contida na primeira planilha do arquivo de excel em que a base é disponibilizada. 
...
Beleza, sabemos agora que a vari?vel de Valor Adicionado a Preços Constantes de 2005 está na moeda local e que a variável com nível de emprego está em milhares. Estamos interessados nelas. 
...
(pwt <- pwt9.1 %>%
  filter(year == 2005) %>% 
  select(isocode, pl_gdpo, xr) %>% # queremos pa?s, n?vel de pre?os e taxa de c?mbio 
  as_tibble() %>%
  filter(pl_gdpo > 0) %>% # removendo dados duvidosos
  rename(Country = isocode) %>%
  mutate(PPP = pl_gdpo * xr) %>% # obtendo o PPP 
  select(-pl_gdpo, -xr)) # temos um par pa?s-c?mbio real em 2005
...
Piadinhas à parte, vamos fazer um experimento? Vamos pegar a Coréia do Sul de 2005 (limitações de dados da base, eu queria ter algo detalhado assim mais recente), manter a produtividade de cada setor igual, mas deixar a distribuição do trabalho igual à do Brasil, um país menos industrializado. O que acontece com a renda per capita?
...
contrafactual <- function(.data, country1, country2, year = 2005, type) {
  ...
  summarise(contrafactual = sum(VAperCapita*empShare1, na.rm = TRUE)) %>%
      mutate(type = "Structure",
             ...
             varPerc = abs(var)/VAperCapitaAgg) %>%
      return()
